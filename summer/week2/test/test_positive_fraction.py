"""Tests for positive_fraction.py."""

# THIS ONLY TO DEMONSTRATE HOW INTENSE YOU MAY WANT TO GET
# IF/WHEN YOU START YOUR OWN COMPANY AND SELL YOUR SOFTWARE
# I ONLY KEPT A FRACTION (get it, fraction) OF THE TEST CODE
# HERE FROM WHAT SHIPPED WITH BUILT-IN fractions.py
...
from positive_fraction import PositiveFraction
...
class FractionTest(unittest.TestCase):
    ...
    def testArithmetic(self):
        ...
        self.assertTypedEquals(10**23, F(10**23, 1) // F(1, 1))
        self.assertEqual(F(2, 3), F(-7, 3) % F(3, 2))
        self.assertEqual(F(8, 27), F(2, 3) ** F(3, 1))
        self.assertEqual(F(27, 8), F(2, 3) ** -3)
        self.assertTypedEquals(2.0, F(4, 1) ** F(1, 2))
    ...
    def testBigFloatComparisons(self):
        ...
        self.assertFalse(F(10**23, 1) == float(10**23))
        # The first test demonstrates why these are important.
        self.assertFalse(1e23 < float(F(math.trunc(1e23) + 1, 1)))
        self.assertTrue(1e23 < F(math.trunc(1e23) + 1, 1))
        self.assertFalse(1e23 <= F(math.trunc(1e23) - 1, 1))
        self.assertTrue(1e23 > F(math.trunc(1e23) - 1, 1))
        self.assertFalse(1e23 >= F(math.trunc(1e23) + 1, 1))
    ...
    @unittest.skip("included here to remind us how to temporarily skip a test")
    def test_slots(self):
        r = F(13, 7)
        self.assertRaises(AttributeError, setattr, r, 'a', 10)

def test_main():
    run_unittest(FractionTest)
